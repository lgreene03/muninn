...
### Security
- **2026-09 dependency-check CVE backlog triage.** The scheduled OWASP dependency-check run (2026-09-11, GH Actions run 34593356302) failed `-DfailBuildOnCVSS=7` against ~60 advisories across 20 dependencies. A first pass was verified against a real dependency-check run on the fix branch (run 34651381704), which caught a mis-keyed suppression and several findings not in the original advisory list (see PR #48 for the full, evidence-based disposition table and the correction history). Final disposition:
  - **Pinned via new `dependencyManagement`:** `io.netty:netty-bom` → `4.2.18.Final` (closes all 20 flagged netty-* CVEs); `commons-configuration2` → `2.15.1` (CVE-2026-45205); `kafka-clients`/`kafka-server`/`kafka-server-common` → `4.3.1` (CVE-2026-41115); `httpclient5` → `5.6.4` and `httpcore5`/`httpcore5-h2` → `5.4.3` (already the resolved versions — pinned for resilience); `tomcat-embed-core`/`tomcat-embed-websocket` → `11.0.25` (9 CVEs up to CVSS 9.8 — Spring Boot 4.1.1's own managed Tomcat version, not caught by the original advisory list, found only once a real scan ran against this branch).
  - **Excluded (dead code path, not just suppressed):** `jetty-webapp`/`jetty-servlet` from `hadoop-common`, and `jline`/`websocket-client`/`websocket-common`/`websocket-api` from `hadoop-mapreduce-client-core` (all on the EOL Jetty 9.4 branch or an ancient jline, all dead paths for a library-only Hadoop consumer that never starts `HttpServer2` or invokes YARN CLI code).
  - **Already fixed on `main`** by prior Dependabot merges before this triage even started: direct `jackson-databind` (2.21.5), the jackson-databind shaded inside `parquet-jackson` (parquet 1.17.1 → 1.18.0 bumped its shaded copy to 2.22.1), `postgresql` (42.7.13), test-scope `log4j-api` (2.25.5), and `parquet-avro`'s own real CVE-2025-46762 finding (1.18.0, well past the fix line).
  - **Suppressed with a dated, reasoned entry** in `dependency-check-suppressions.xml`: `hive-storage-api` CVEs shaded into `orc-core` (verified the shaded classes are decimal-type helpers only, not the vulnerable HiveServer2/LLAP/credential code — re-keyed once already, see below); `opentelemetry-api`/`opentelemetry-sdk` CVE-2026-54285 (filed against opentelemetry-js, not either Java artifact); `kotlin-stdlib`/`kotlin-stdlib-common` CVE-2026-53914 and CVE-2020-29582 (compiler-only / already-exceeded-fix-version, and unreachable); `parquet-avro` CVE-2023-37475 (filed against an unrelated Go library, `hamba/avro`); `iceberg-parquet` CVE-2025-46762 (a *separate*, false-positive finding against iceberg-parquet's own coordinate — dependency-check matched its version number against the wrong "apache:parquet" CPE; the earlier claim that this CVE was "already fixed" for iceberg-parquet conflated it with parquet-avro's distinct, genuinely-fixed finding — corrected here).
  - **Suppressed as a genuine, currently-unpatched residual risk** (not a false positive): `parquet-avro`/`parquet-column` CVE-2026-73334 (KMS-URL host validation gap in `org.apache.parquet.crypto.keytools`, versions 1.12–1.18). No fixed Parquet release exists yet (1.19 not yet published). Suppressed on the grounds that Muninn never uses Parquet column encryption / KMS client plugins today — flagged explicitly for revisit if that ever changes.
  - **Corrected:** the original triage mis-keyed the `hive-storage-api` suppression's `packageUrl` against `org.apache.orc/orc-core` instead of the actually-reported `org.apache.hive/hive-storage-api` coordinate, so it had zero matches and all four CVEs kept firing. Re-keyed once a real scan surfaced the "Unused Suppression Rule Analyzer" warning.
